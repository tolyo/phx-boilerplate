...
async function readResponseBody(response) {
  ...
  while (!result.done) {
    // Decode the chunk of data and concatenate it into a string
    data += decoder.decode(result.value, { stream: true });
    result = await reader.read();
  }

  // Once all data is read, return or process it further
  return data;
}
...
export function initRouterWithBase(baseUrl, routes) {
  BASE_URL = baseUrl;
  initRouter(routes);
}
